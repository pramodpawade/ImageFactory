...
control 'motd-02' do
  ...
  describe file ('/etc/motd') do
    its('content') { should match '\W*This is a monitored confidential of My Ditch Bank System\W*'}
  end
end
...
control 'Check AWS Inspector agent exists or not' do
  impact 1.0
  title 'client: Check AWS Inspector agent exists or not'
  desc 'Check AWS Inspector agent exists or not'
  describe file ('/opt/aws/awsagent/bin/awsagent') do
    it { should exist }
    its('type') { should cmp 'file' }	
  end
end
